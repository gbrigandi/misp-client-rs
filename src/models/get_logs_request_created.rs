...
#[serde(untagged)]
pub enum GetLogsRequestCreated {
    /// You can use any of the valid time related filters (examples: 7d, timestamps, [14d, 7d] for ranges, etc.)
    DateRestSearchFilter(String),
    /// Interval described by two dates
    DateIntervalRestSearchFilter(Vec<String>),
}

impl Default for GetLogsRequestCreated {
    fn default() -> Self {
        Self::DateRestSearchFilter(Default::default())
    }
}
