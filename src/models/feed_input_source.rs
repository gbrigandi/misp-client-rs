...
/// FeedInputSource : Specify whether the source (url field) is a directory (local) or an genuine url (network).
/// Specify whether the source (url field) is a directory (local) or an genuine url (network).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum FeedInputSource {
    #[serde(rename = "local")]
    Local,
...
impl std::fmt::Display for FeedInputSource {
...
impl Default for FeedInputSource {
    fn default() -> FeedInputSource {
        Self::Local
    }
}
