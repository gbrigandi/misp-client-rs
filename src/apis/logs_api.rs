...
pub async fn get_logs(configuration: &configuration::Configuration, get_logs_request: Option<models::GetLogsRequest>) -> Result<Vec<models::GetLogs200ResponseInner>, Error<GetLogsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_get_logs_request = get_logs_request;

    let uri_str = format!("{}/admin/logs", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_get_logs_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::GetLogs200ResponseInner&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::GetLogs200ResponseInner&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
