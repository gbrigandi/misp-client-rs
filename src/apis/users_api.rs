...
/// struct for typed errors of method [`delete_user_totp`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum DeleteUserTotpError {
    Status403(models::UnauthorizedApiError),
    Status404(models::NotFoundUserTotpDeleteError),
    DefaultResponse(models::ApiError),
    UnknownValue(serde_json::Value),
}
...
/// struct for typed errors of method [`reset_user_password`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn add_user(configuration: &configuration::Configuration, user_no_id: Option<models::UserNoId>) -> Result<models::User, Error<AddUserError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_user_no_id = user_no_id;

    let uri_str = format!("{}/admin/users/add", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_user_no_id);
...
pub async fn delete_user(configuration: &configuration::Configuration, user_id: &str) -> Result<models::DeleteUser200Response, Error<DeleteUserError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_user_id = user_id;

    let uri_str = format!("{}/admin/users/delete/{userId}", configuration.base_path, userId=crate::apis::urlencode(p_user_id));
...
pub async fn delete_user_totp(configuration: &configuration::Configuration, user_id: &str) -> Result<models::DeleteUserTotp200Response, Error<DeleteUserTotpError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_user_id = user_id;

    let uri_str = format!("{}/users/totp_delete/{userId}", configuration.base_path, userId=crate::apis::urlencode(p_user_id));
    let mut req_builder = configuration.client.request(reqwest::Method::DELETE, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::DeleteUserTotp200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::DeleteUserTotp200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<DeleteUserTotpError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

pub async fn edit_user(configuration: &configuration::Configuration, user_id: &str, user: Option<models::User>) -> Result<models::User, Error<EditUserError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_user_id = user_id;
    let p_user = user;

    let uri_str = format!("{}/admin/users/edit/{userId}", configuration.base_path, userId=crate::apis::urlencode(p_user_id));
...
pub async fn get_user_by_id(configuration: &configuration::Configuration, user_id: &str) -> Result<models::ExtendedUser, Error<GetUserByIdError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_user_id = user_id;

    let uri_str = format!("{}/admin/users/view/{userId}", configuration.base_path, userId=crate::apis::urlencode(p_user_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ExtendedUser`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_users(configuration: &configuration::Configuration, ) -> Result<Vec<models::UserListItem>, Error<GetUsersError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::UserListItem&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::UserListItem&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn reset_user_password(configuration: &configuration::Configuration, user_id: &str, first_time_reset: &str) -> Result<models::ResetUserPassword200Response, Error<ResetUserPasswordError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_user_id = user_id;
    let p_first_time_reset = first_time_reset;

    let uri_str = format!("{}/users/initiatePasswordReset/{userId}/{firstTimeReset}", configuration.base_path, userId=crate::apis::urlencode(p_user_id), firstTimeReset=crate::apis::urlencode(p_first_time_reset));
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ResetUserPassword200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ResetUserPassword200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ResetUserPasswordError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
